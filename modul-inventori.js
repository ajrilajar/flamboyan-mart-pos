...
export function renderInventori() {
    ...
    content.innerHTML = `
        <div id="view-list" class="flex flex-col gap-4 max-w-2xl mx-auto p-4 animate-fadeIn">
            ...
            <div id="list-barang" class="grid grid-cols-1 gap-4 pb-32"></div>
            <button onclick="window.bukaHalamanEdit(null)" class="fixed bottom-24 right-4 sm:right-[calc(50%-20rem)] lg:right-[calc(50%-20rem)] bg-emerald-500 text-white px-6 py-4 rounded-full shadow-2xl flex items-center gap-3 font-bold z-40 border-4 border-white active:scale-95 transition-all">
        ...
        <div id="view-detail" class="hidden fixed inset-0 bg-white z-[60] overflow-y-auto pb-32">
            <div class="max-w-2xl mx-auto min-h-screen bg-white shadow-xl">
                ...
                <div class="fixed bottom-0 left-0 right-0 bg-white border-t flex justify-center z-20">
                    <div class="w-full max-w-2xl p-4 flex gap-3">
        ...
        <div id="view-edit" class="hidden fixed inset-0 bg-gray-50 z-[70] overflow-y-auto pb-24">
            <div class="max-w-2xl mx-auto min-h-screen bg-gray-50 shadow-xl">
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div onclick="window.bukaPickerKategori()" class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex justify-between items-center cursor-pointer active:bg-gray-50 transition-all">
                        <div>
                ...
                <div class="p-4 fixed bottom-0 left-0 right-0 bg-gray-50 border-t flex justify-center">
                    <button onclick="window.simpanPerubahan()" class="w-full max-w-2xl bg-emerald-500 text-white py-4 rounded-xl font-bold text-lg shadow-lg active:scale-95 transition-all uppercase">Simpan Perubahan</button>
                </div>
            </div>
        </div>

        <div id="picker-kategori" class="hidden fixed inset-0 bg-black/50 z-[100] flex items-end justify-center overflow-hidden">
            <div class="bg-white w-full max-w-2xl rounded-t-3xl animate-slide-up flex flex-col h-[85vh]">
                ...
                <div id="view-buat-kategori" class="hidden flex flex-col h-full p-6 animate-fadeIn">
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            ...
                            <input type="text" id="input-kategori-baru" class="w-full p-4 border-2 border-emerald-500 rounded-xl outline-none text-gray-700 font-medium" placeholder="Contoh: Rokok">
                        </div>
                    </div>
                    <div class="pt-6">
    ...
    // Inisialisasi logika tombol simpan (Gambar 6)
    setTimeout(() => {
...
// LOGIKA NAVIGATION & CRUD TETAP SAMA (Hanya Wrapper UI yang Berubah)
...
window.filterInventori = () => {
    ...
    Object.entries(databaseBarang).forEach(([id, item]) => {
        if (item.nama.toLowerCase().includes(keyword)) {
            ...
            listDiv.innerHTML += `
                <div onclick="window.bukaDetail('${id}')" class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm active:bg-gray-50 flex flex-col gap-3 transition-all cursor-pointer">
                    <div class="flex gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <span class="text-[10px] bg-gray-100 text-gray-400 px-2 py-0.5 rounded font-bold uppercase tracking-tight border border-gray-200">${item.kategori || 'Umum'}</span>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-2 border-t pt-3">
                        <div><p class="text-[9px] text-gray-400 font-bold uppercase mb-0.5 tracking-tighter">Jual</p><p class="text-xs font-bold text-gray-700">Rp ${Number(item.harga_jual).toLocaleString()}</p></div>
                        <div><p class="text-[9px] text-gray-400 font-bold uppercase mb-0.5 tracking-tighter">Beli</p><p class="text-xs font-bold text-gray-700">Rp ${Number(item.harga_beli).toLocaleString()}</p></div>
                        <div class="text-right"><p class="text-[9px] text-gray-400 font-bold uppercase mb-0.5 tracking-tighter">Stok</p><p class="text-xs font-bold ${item.stok <= 0 ? 'text-rose-500' : 'text-emerald-700'} font-black">${item.stok} ${item.satuan}</p></div>
                    </div>
                </div>`;
        }
    });
    ...
    document.getElementById('detail-render').innerHTML = `
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-gray-50 p-3 rounded-xl border border-gray-100">
                <p class="text-[9px] text-gray-400 font-bold uppercase mb-1 tracking-tighter">Jual</p>
                <p class="text-sm font-bold">Rp ${Number(item.harga_jual).toLocaleString()}</p>
            </div>
            <div class="bg-gray-50 p-3 rounded-xl border border-gray-100">
                <p class="text-[9px] text-gray-400 font-bold uppercase mb-1 tracking-tighter">Beli</p>
                <p class="text-sm font-bold">Rp ${Number(item.harga_beli).toLocaleString()}</p>
            </div>
            <div class="bg-emerald-50 p-3 rounded-xl border border-emerald-100 text-right">
                <p class="text-[9px] text-emerald-600 font-bold uppercase mb-1 tracking-tighter">Nilai Stok</p>
                <p class="text-sm font-bold text-emerald-700">Rp ${(item.stok * item.harga_beli).toLocaleString()}</p>
            </div>
        </div>
...
// PICKER & EDIT (LOGIKA TIDAK BERUBAH)
window.bukaPickerKategori = () => { document.getElementById('picker-kategori').classList.remove('hidden'); window.switchKategoriView('view-pilih-kategori'); window.renderKategoriList(); };
window.tutupPickerKategori = () => document.getElementById('picker-kategori').classList.add('hidden');
window.switchKategoriView = (viewId) => { document.getElementById('view-pilih-kategori').classList.add('hidden'); document.getElementById('view-buat-kategori').classList.add('hidden'); document.getElementById(viewId).classList.remove('hidden'); };
...
window.prosesTambahKategori = () => { const input = document.getElementById('input-kategori-baru'); if(input.value.trim()){ if(!daftarKategori.includes(input.value.trim())) daftarKategori.unshift(input.value.trim()); window.pilihKategori(input.value.trim()); input.value = ""; } };
